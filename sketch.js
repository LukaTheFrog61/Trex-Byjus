...
function setup() {
  ...
  nuvem = createGroup();
  cactos = createGroup();
  chao = createSprite(600, 190, 10, 10);
  chao.addImage(chaoimg);
  chao.velocityX = - 7;
  ground = createSprite(50, 200,600, 20);
  ...
  //trex.debug = true;
  gameover = createSprite(594.25,100,10,10);
  gameover.addImage(gameoverimg);
  restart = createSprite(594.25,150,10,10);
...
function draw() {
  ...
  if (gamestate == "jogando") {
    ...
    if (trex.y > 160 && (keyDown("SPACE") || keyDown(UP_ARROW))) {
      trex.velocityY = - 15;
      jumpsound.play();
    }
...
function spawnN() {
  if (frameCount % 40 == 0) {
    var nuvem1;
    nuvem1 = createSprite(width + 50, Math.round(random(10, 100)), 10, 10);
    nuvem1.lifetime = 140;
...
function spawnC() {
  if (frameCount % 60 == 0) {
    var cactos1;
    cactos1 = createSprite(width + 40, 180, 20, 5);
    cactos1.lifetime = 280;
